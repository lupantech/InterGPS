...
from sympy import  cos, sin, sqrt, pi, acos, asin, solve
...
class ExtendedDefinition(BasicDefinition):
    ...
    def parseAngle(self, angle):
        if len(angle) == 3:
            ...
            valid_y = [y for y in points[1:] if not y in line]
            for center in self.find_all_circles():
                if len(valid_y) > 1 and center in valid_y:
                    valid_y.remove(center)
    ...
    def parseArc(self, arc):
        ...
        '''
        assert len(arc) >= 2 and len(arc) <= 4
    ...
    def defineCongruentTriangle(self, tri1, tri2):
        ...
        for ch in range(3):
            self.angleEqual([tri1[ch], tri1[(ch+1)%3], tri1[(ch+2)%3]], [tri2[ch], tri2[(ch+1)%3], tri2[(ch+2)%3]])
    ...
    def delete_unused_points(self, points):
        ...
        for i in range(len(self.relations)):
            ...
            for fact in facts_:
                if all([not point in fact for point in points]):
            ...
            for i in range(l):
                for j in range(i+1, l):
                    ...
                    for point in intersection:
                        if (polygon[i], point, polygon[(i+1)%l]) in f or (polygon[j], point, polygon[(j+1)%l]) in f:
                            is_polygon = False
            if is_polygon:
    ...
    def expand_angles(self):
        ...
        for angle in angles:
            ...
            for point1, point2 in product(_1, _2):
                ...
                same_side1 = (angle[1], angle[0], point1) in f or (angle[1], point1, angle[0]) in f or point1 == angle[0]
                same_side2 = (angle[1], angle[2], point2) in f or (angle[1], point2, angle[2]) in f or point2 == angle[2]
    ...
    def cross(self, p0, p1, p2):
        if self.point_positions == None: return 0
    ...
    def set_angle_sum(self):
        ...
        for angle in angles:
            ...
            for point in points:
                ...
                if self.point_positions == None: continue
